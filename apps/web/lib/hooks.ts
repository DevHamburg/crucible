...
export const useSafetyReport = (runId: string) =>
  useQuery({ queryKey: ["safety-report", runId], queryFn: () => api.get<any>(`/safety/report/${runId}`), enabled: !!runId });
...
export const useTournament = (id: string, poll = false) =>
  useQuery({ queryKey: ["tournament", id], queryFn: () => api.get<Tournament>(`/arena/tournaments/${id}`), enabled: !!id, refetchInterval: poll ? 2000 : false });
