...
async def execute_tournament(tournament_id: str) -> None:
    ...
    except Exception as exc:  # noqa: BLE001 — persist partial bracket + terminate the stream
        error_msg = str(exc) or exc.__class__.__name__

    async with SessionLocal() as s:
        t = await s.get(Tournament, tournament_id)
        if t is not None:
            t.status = "error" if error_msg else "completed"
            t.champion = champion
            t.bracket = {"rounds": bracket_rounds, **({"error": error_msg} if error_msg else {})}
            await s.commit()
    # Always publish the terminal event so live streams stop waiting.
    await bus.publish(channel, {
        "type": "tournament_completed", "id": tournament_id, "champion": champion,
